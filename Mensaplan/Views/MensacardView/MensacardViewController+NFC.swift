//
//  MensacardViewController+NFC.swift
//  Mensaplan
//
        ...
        if case let NFCTag.miFare(tag) = tags.first! {
            session.connect(to: tags.first!) { (error: Error?) in
                if(error != nil) {
                    ...
                    self.hapticsGenerator.notificationOccurred(.error)
                    return
                }
            ...
            session.invalidate(errorMessage: "Die gescannte Karte konnte nicht gelesen werden.\nVersuche es bitte erneut.")
        }
    }
